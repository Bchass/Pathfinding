  ...
  animate(visited, shortest) {
    for (let i = 0; i <= visited.length; i++) {
      ...
      setTimeout(() => {
        ...
        document.getElementById(`node-${node.rows}-${node.columns}`).className =
          "node node-visited";
      }, 20 * i);
    }
  }
                  ...
                  {rows.map((node, nodeIdx) => {
                    ...
                    return (
                      <Node
                        ...
                        onMouseDown={(rows, columns) =>
                          this.MouseDown(rows, columns)
                        }
                        onMouseEnter={(rows, columns) =>
                          this.MouseEnter(rows, columns)
                        }
